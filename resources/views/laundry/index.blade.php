...
@endif
                    ...
                    <div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 border border-white/30 min-w-[180px]" 
                        ...
                        <p class="text-white/70 text-xs font-medium mb-1">
                            @if($voucher->discount_type == 'percent')
                                ...
                                @if($voucher->max_discount) (Max Rp {{ number_format($voucher->max_discount/1000) }}rb) @endif
                            @else
                                Potongan Rp {{ number_format($voucher->discount_value/1000) }}rb
                            @endif
                        </p>
                        <div class="flex items-center gap-2">
                        ...
                        <p class="text-white/60 text-xs mt-1">Min. order Rp {{ number_format($voucher->min_order/1000) }}rb</p>
                        @endif
                    </div>
                    @endforeach
                </div>
                <a href="{{ route('vouchers.index') }}" class="bg-white text-blue-600 font-bold px-6 py-3 rounded-xl hover:bg-yellow-300 hover:text-slate-900 transition shadow-lg">
                    Lihat Semua Voucher →
                </a>
            </div>
        </div>
    ...
    <div id="reservasi" class="py-24 bg-slate-50 relative overflow-hidden reveal"
        x-data="{
            type: 'dropoff',
            items: [],
            services: @js($services->map(fn($s) => ['id' => $s->id, 'name' => $s->name, 'price' => $s->price])),
            selectedService: '',
    ...
    >
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="bg-white rounded-[2.5rem] shadow-2xl p-10 lg:p-14 border border-slate-100 transition-colors duration-300">
                ...
                <form action="{{ route('transaction.store') }}" method="POST" x-ref="orderForm">
                    @csrf
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                @auth
                                    ...
                                    @endphp
                                    @if($defaultPhone)
                                        <div x-data="{ useDefault: true, phoneValue: '{{ $defaultPhone }}' }">
                                            <div class="flex items-center gap-2">
                        ...
                        <div x-show="type === 'pickup_delivery'" x-transition class="space-y-2">
                            ...
                            @auth
                                @if($addresses->count() > 0)
                                    ...
                                    <a href="{{ route('addresses.index') }}" target="_blank" class="text-xs text-blue-600 hover:underline">
                                        📍 Kelola Alamat Saya
                                    </a>
                                @else
                                    ...
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mb-2">
                                        <p class="text-xs text-yellow-800">
                                            ...
                                            <a href="{{ route('addresses.index') }}" target="_blank" class="font-bold underline">Tambah alamat</a> 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                @forelse($reviews as $review)
                    <div class="bg-slate-50 p-8 rounded-3xl border border-slate-100 hover:shadow-xl transition duration-300">
                        ...
                        <p class="text-slate-600 mb-6 leading-relaxed">"{{ $review->comment ?? 'Pelayanan sangat memuaskan!' }}"</p>
                        <div class="flex items-center gap-4">

...
@section('title', 'Voucher & Promo - Laundry U9')
...
<div class="min-h-screen bg-gradient-to-b from-slate-50 to-white py-12">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-slate-100 hover:shadow-xl transition duration-300"
                ...
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 text-white relative">
                    ...
                    <div class="flex items-center gap-3 mt-1">
                        <span class="text-2xl font-black tracking-wider">{{ $voucher->code }}</span>
                ...
                <div class="px-6 py-5">
                            ...
                            <p class="text-xl font-black text-slate-900">
                                @if($voucher->discount_type == 'percent')
                                    ...
                                    @if($voucher->max_discount)
                                        <span class="text-sm font-normal text-slate-400">(Max Rp {{ number_format($voucher->max_discount, 0, ',', '.') }})</span>
                                    @endif
                                @else
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-slate-700">Rp {{ number_format($voucher->min_order, 0, ',', '.') }}</span>
                        </div>
                        @endif
                        <div class="flex justify-between">
                            <span class="text-slate-400">Berlaku Hingga</span>
                            <span class="font-bold text-slate-700">{{ $voucher->valid_until->format('d M Y') }}</span>
                        </div>
                        @if($voucher->quota)
                        <div class="flex justify-between">
                            <span class="text-slate-400">Sisa Kuota</span>
                            <span class="font-bold {{ ($voucher->quota - $voucher->used_count) <= 5 ? 'text-red-500' : 'text-green-600' }}">
                ...
                <div class="bg-slate-50 px-6 py-3 border-t border-slate-100">
                    <a href="{{ route('home') }}#order-form" class="text-blue-600 font-bold text-sm hover:underline flex items-center gap-1">
                        Gunakan Sekarang →
                    </a>
                </div>
            </div>
        ...
        <div class="text-center mt-12">
            <a href="{{ route('home') }}" class="inline-flex items-center gap-2 bg-slate-900 text-white font-bold px-8 py-4 rounded-xl hover:bg-slate-800 transition shadow-lg">
                ← Kembali ke Beranda
            </a>
        </div>
    </div>

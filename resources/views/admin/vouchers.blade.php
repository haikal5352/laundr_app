                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="font-mono font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">{{ $voucher->code }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($voucher->discount_type == 'percent')
                                            <span class="text-green-600 font-bold">{{ $voucher->discount_value }}%</span>
                                            @if($voucher->max_discount)
                                                <span class="text-gray-400 text-xs">(Max {{ number_format($voucher->max_discount) }})</span>
                                            @endif
                                        @else
                                            <span class="text-green-600 font-bold">Rp {{ number_format($voucher->discount_value) }}</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                        {{ $voucher->min_order ? 'Rp ' . number_format($voucher->min_order) : '-' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        @if($voucher->quota)
                                            <span class="{{ $voucher->used_count >= $voucher->quota ? 'text-red-500' : 'text-gray-600' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex gap-2">
                                            <button onclick="editVoucher({{ $voucher->id }}, '{{ $voucher->code }}', '{{ $voucher->discount_type }}', {{ $voucher->discount_value }}, {{ $voucher->min_order ?? 0 }}, {{ $voucher->max_discount ?? 0 }}, {{ $voucher->quota ?? 'null' }}, '{{ $voucher->valid_from->format('Y-m-d') }}', '{{ $voucher->valid_until->format('Y-m-d') }}', {{ $voucher->is_active ? 'true' : 'false' }})"
